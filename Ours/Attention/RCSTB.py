...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=2, embed_dim=96, norm_layer=None):
        super().__init__()
...
class RES_SWIN(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=1, in_chans=2,after_chans=2,
                 embed_dim=96, depths=[2,2], num_heads=[6,6],
        ...
        self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1,dilation=1)
        self.conv_after_RSTB = nn.Conv2d(embed_dim, num_after_ch, 3, 1, 1,dilation=1)
    ...
    def forward(self,x):
        ...
        x = self.patch_embed(self.conv_first(x))
        if self.ape:
...
# my_res_swin=RES_SWIN()
# tensor=torch.randn(1,2,256,256)
# answer=my_res_swin(tensor)
# print("ok")
